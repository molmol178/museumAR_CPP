...
namespace TopologyFeature{
    //calcCentroid用の構造体
    typedef struct value_xy_t{
      int value; //画素値
      Point focus_pt; //座標
    } value_xy_t;

    typedef struct centroids_t{
      int value;//画素値
    ...
    vector<vector<int> > featureDetection(int patch_size, Mat changed_label_img,  vector<vector<int> > *sum_one_dimention_scanning, vector<vector<float> > *sum_xy, vector<vector<int> > *sum_boundary, vector<vector<double> > *sum_ave_keypoint, vector<vector<int> > *sum_mean_vector);
    ...
    void writeFeaturePoint(Mat template_img, vector<vector<float> > *sum_xy, vector<vector<int> > *sum_boundary, string filepath);
    ...
    vector<centroids_t> calcCentroids(string centroids_path, Mat img);
